...
int sqrt_int(int target, int cur, int a, int b) {
  ...
  return diff > 0? sqrt_int(target, (b + cur + 1)/2, cur + 1, b) : sqrt_int(target, (a + cur - 1)/2, a, cur - 1);
}

int mysqrt(int val) {
  return sqrt_int(val, val/2, 0, val);
}
